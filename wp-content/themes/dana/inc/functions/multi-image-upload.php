...
class Multi_Image_Uploader {
    ...
    public function add_meta_box($post_type) {
        ...
        if (in_array($post_type, $this->post_types)) {
            add_meta_box(
                    'gallery_images',
                    __('تصاویر گالری', 'miu_textdomain'),
    ...
    public function render_meta_box_content($post) {
        ...
        if ($value && is_array($value)) {
            foreach ($value as $image) {
                // $script.="addRow('{$image}');";
                if ($image && $image->id) {
                    echo '<div id="row-miu-'.$image->id.'" style=\'margin-bottom: 20px;\'><img width="60" height="60" src="'. $image->thumbnail .'" class="attachment-post-thumbnail" alt="'.$image->filename.'" style=\'vertical-align: top;\' /><h4 style=\'text-align: center; display: inline-block; width: 180px;margin-left: 10px;\'>'.$image->title.'</h4><a class="miu_remove_image_button" id="miu_row-'. $image->id .'">حذف</a><input type="hidden" name="miu_images[]" value="'. urlencode(json_encode($image)) .'" /></div>';
                }
            }
        }

        echo '</div><input id="miu_upload_image_button" type="button" value="افزودن جدید ..." class="button" />';
    }
    ...
    function enqueue_scripts($hook) {
        ...
        wp_enqueue_script('miu_script', get_bloginfo('template_url') . '/inc/functions/miu_script.js', array('jquery'), null, true);
    }
}
