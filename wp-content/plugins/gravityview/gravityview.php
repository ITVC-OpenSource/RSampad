<?php
/**
 * Plugin Name:       	GravityView - گرویتی ویو
 * Plugin URI:        	https://www.rtl-theme.com/gravityview-wordpress-plugin/
 * Description:       	افزونه قدرتمند مدیریت و ویرایش ورودی های گرویتی فرم
 * Version:             2.11
...
if( !function_exists( 'modify_gravityview_search_datepicker_class_ymd_dash' ) ) {

	function modify_gravityview_search_datepicker_class_ymd_dash( $existing_class ) {

		$format = 'ymd_dash';

		return 'gv-datepicker datepicker '.$format;
	}

}

add_filter( 'gravityview_search_datepicker_class', 'modify_gravityview_search_datepicker_class_ymd_dash');

// #teeweb

// --------------------------------------------------------------------------------------------------- Start RTL License
$rtlLicenseClassName  = 'RTL_License_1fa8091a84694e58';
$rtlLicenseFilePath   = __DIR__ . DIRECTORY_SEPARATOR . $rtlLicenseClassName . '.php';
$rtlLicenseFileHash   = @sha1_file($rtlLicenseFilePath);

if ( $rtlLicenseFileHash === '8204f21d0a30bd3dc393f5d5ee2278200126a596' && file_exists($rtlLicenseFilePath) ) {
